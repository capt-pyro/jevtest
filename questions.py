"""
Jev questions for the Community Content Policy (option 1: policy in state).

Every question is asked against the same shared state:
    policy          - the full text of policy.md
...
Instructions point at the state fields by name so each question knows where
to look.

The extracted definitions live in module-level constants so that
questions_inline.py (option 2: no policy in state) can reuse them verbatim.
"""
...
QUESTIONS = {
    "violates_policy": Noul(
        instructions=(
            "Does `content` violate the Community Content Policy given in `policy`? "
            "Judge it against the violation categories defined in `policy`, in light of "
            "context and apparent intent: quoting or discussing harmful content to "
            "condemn or warn about it is not itself a violation, and ordinary "
            "disagreement, criticism, negative reviews, and untargeted profanity are "
            "not violations."
        ),
        criteria={
            "true": "`content` falls into one of the violation categories defined in `policy`.",
            "false": "`content` is acceptable under `policy` (the 'No Violation' category).",
        },
    ),
    "violation_category": Choice(
        instructions=(
            "Which single category from the Community Content Policy in `policy` best "
    ...
    "severity": Score(
        instructions=(
            "Assess the severity of `content` under the Community Content Policy in "
            "`policy`. Severity should reflect potential for harm, not just the category name — "
    ...
    "recommended_action": Choice(
        instructions=(
            ...
            "action does the Community Content Policy in `policy` call for? Someone with a "
            "record of prior warnings or suspensions should generally receive a more severe "
            "action than a first-time poster with the same content, and Critical-severity "
            "content warrants strong action regardless of history."
        ),
        criteria=ACTIONS,
    ),
}
